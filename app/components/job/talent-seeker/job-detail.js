...
import {
  ...
  defaultHeaderStyle,
  defaultHeaderWithShadowStyle,
} from '@styles/components/transparentHeader.style';
...
class JobDetail extends Component {
  constructor(props) {
    ...
    // const { navigate, goBack, state } = this.props.navigation;
    this._applyJob = this._applyJob.bind(this);
  }
  ...
  static navigationOptions = ({navigation}) => ({
    ...
    headerVisible: false,
    headerTitle:
      navigation.state.params.job.title ||
      _.head(navigation.state.params.job.job_detail).sub_reference_detail.title,
    headerLeft: <ButtonBack isGoBack={navigation} btnLabel="" />,
    headerStyle: defaultHeaderWithShadowStyle,
    headerRight1: (
      <ButtonTextRight callBack={navigation.state.params} btnLabel="Apply" />
    ),
      ...
      !navigation.state.params.can_remove ? (
        <View style={[styles.flexVerMenu, styles.flexCenter]}>
          ...
          <TouchableOpacity
            ...
            navigation.state.params.isLoadingOnHeader ? (
              <ActivityIndicator
                animating={true}
                style={[]}
                size="small"
              ...
              <View>
                {navigation.state.params && navigation.state.params.applied ? (
                  <Icon name="done" style={[styles.doneIcon]} />
                ) : (
                  <Text style={[styles.txtRightHeader]}> Apply </Text>
                )}
              </View>
            )}
          </TouchableOpacity>
        ...
        <View style={[styles.flexVerMenu, styles.flexCenter]}>
          <TouchableOpacity
            style={[
              {marginRight: 5},
              navigation.state.params &&
                navigation.state.params.applied && {marginRight: 15},
            ]}
            onPress={() => {
            ...
            navigation.state.params.isLoadingOnHeader ? (
              ...
              <View>
                {navigation.state.params && navigation.state.params.applied ? (
                  ...
                  <Text style={[styles.txtRightHeader]}>
                    {' '}
                    <Icon
                      name="delete"
                      style={[styles.doneIcon, {color: 'gray', fontSize: 24}]}
                    />{' '}
                  </Text>
                )}
              </View>
        ...
        try {
          GoogleAnalyticsHelper._trackEvent('Job', 'Apply', {
            title:
              ...
              _.head(this.state.jobInfo.job_detail).sub_reference_detail.title,
            desc: this.state.jobInfo.description
              ? this.state.jobInfo.description
              : '',
            gender: Helper._getGenderJob(this.state.jobInfo.criteria.gender),
            age:
              this.state.jobInfo.criteria.min_age +
              '-' +
              this.state.jobInfo.criteria.max_age,
  ...
  _prepareRemoveJob = () => [
    Alert.alert('This job application will be removed.', '', [
      {
        text: 'Cancel',
  ...
  _removeJob = () => {
    ...
    setParams({isLoadingOnHeader: true});
    let API_URL =
      '/api/jobs/' + this.state.jobInfo.job_detail[0]._id + '/withdraw';
    postApi(API_URL).then(response => {
      if (response.code == 200) {
        try {
  ...
  _getKinds = item => {
    if (item.criteria) return item.criteria.type;
    else return [];
  };
  ...
  _getCover = () => {
    ...
    return _.head(_objDetail.reference_detail)
      ? {uri: _.head(_objDetail.reference_detail).thumbnail_url_link}
      : require('@assets/banner-default.jpg');
  };
  ...
  viewProfile = _item => {
    ...
    GoogleAnalyticsHelper._trackEvent('View Profile', 'Job Detail', {
      user_id: _item._id,
      user_name: Helper._getUserFullName(this.state.jobInfo),
    });
      ...
      <ScrollView removeClippedSubviews={false} style={[styles.wrapper]}>
        <View style={[styles.topSection]}>
          ...
          {false && this.props.navigation.state.params.job.status != 'cancel' && (
            <TouchableOpacity
              onPress={() =>
                navigate('SendJobByChat', {
                  shareJob: true,
                  shareJobInfo: _sendJobInfo,
                })
              }
        ...
        <View style={[styles.middleSection, {padding: 20}]}>
              ...
              {this._getKinds(this.state.jobInfo).map((item, index) => {
                return (
                  <TouchableOpacity
                    ...
                    key={index}
                    style={[
                      styles.tagsSelectNormal,
                      styles.withBgPinkOld,
                      styles.tagsSelectAutoWidth,
                      styles.noMargin,
                      styles.marginTopXXS,
                    ]}>
                    <Text
                      style={[
                        styles.tagTitleSelected,
                        styles.btFontSize,
                        styles.tagTitleSizeSM,
                      ]}>
          ...
          <View style={[{flexDirection: 'row', paddingVertical: 20}]}>
            <View style={[{flex: 1}]}>
              <Text>Gender</Text>
              <Text style={[{fontSize: 18, fontWeight: 'bold'}]}>
                {Helper._getGenderJob(this.state.jobInfo.criteria.gender)}
              </Text>
            </View>
            <View style={[{flex: 1}]}>
              <Text>Age</Text>
            ...
            <View style={[{flex: 2}]}>
              <Text>Location</Text>
              <Text style={[{fontSize: 18, fontWeight: 'bold'}]}>
                {this.state.jobInfo.criteria.country}
              </Text>
            </View>
          </View>
          <View style={[{paddingVertical: 30}]}>
            ...
            <TouchableOpacity
              activeOpacity={0.8}
              onPress={() =>
                this.viewProfile(this.state.jobInfo.owner_profile)
              }>
              <View style={[styles.userInfo, {marginTop: 15}]}>
                <View style={[styles.avatarContainer]}>
                  ...
                  <CustomCachedImage
                    ...
                    component={ImageProgress}
                    source={{
                      uri: Helper._getCover(this.state.jobInfo.owner_profile),
                    }}
                    indicator={ProgressCircle}
                  />
                </View>
                ...
                <Text style={[styles.userName, {fontWeight: 'bold'}]}>
                  {Helper._getUserFullName(this.state.jobInfo)}
                </Text>
                {/*</View>*/}
              </View>
            </TouchableOpacity>
