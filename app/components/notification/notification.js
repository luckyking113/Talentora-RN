...
class Notification extends Component {
  ...
  _getNoti = (_isLoadMore = false) => {
    ...
    let API_URL =
      '/api/notifications?scope=_created_by&offset=' +
      _offset +
      '&limit=' +
      this.state.limit;
  ...
  _renderItem = ({item}) => {
    return <NotiRowItem {...item} rowPress={this._onRowPress} />;
  };
        ...
        getApi(REQ_API).then(_response => {
          if (_response.code == 200) {
            ...
            navigate('JobDetail', {
              ...
              view_only:
                _response.result.status == 'cancel'
                  ? _response.result.removable
                    ? false
                    : true
                  : false,
              can_remove:
  ...
  render() {
    if (this.state.data.length <= 0 && !this.state.isFirstLoad)
      return <NormalListItemDataMockUpLoading />;
    else {
      ...
      return (
        <View style={[styles.justFlexContainer, styles.mainScreenBg]}>
          ...
          {_.isEmpty(this.state.data) && (
            <View
              style={[
                styles.fullWidthHeightAbsolute,
                {
