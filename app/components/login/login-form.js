...
export default class LogInForm extends Component {
  ...
  _verifyFullFillInfo = _userInfo => {
    if (_userInfo.activeUserRoles) {
      if (_userInfo.profile.attributes) {
        if (
          !_.isEmpty(_userInfo.profile.attributes.kind) &&
          !_.isEmpty(_userInfo.profile.attributes.date_of_birth)
        )
          return true;
        else return false;
      } else {
    ...
    if (_userInfo.activeUserRoles.length <= 0) {
      ...
      if (_userInfo.profile.attributes) {
        if (_.isEmpty(_userInfo.profile.attributes.kind)) {
          ...
          else navigate('TalentSeekerCategory', {noBackButton: true});

          // return true;
        } else if (_.isEmpty(_userInfo.profile.attributes.date_of_birth)) {
          navigate('TalentWelcome', {
            sign_up_info: _userInfo,
            noBackButton: true,
          });
          // return false;
        } else {
  ...
  _saveUserData = response => {
    ...
    if (
      ...
      this._verifyFullFillInfo(_userInfo)
    ) {
  ...
  _goSignUp = response => {
    ...
    const {
      ...
      setParams,
      state,
    } = this.props.loginFrm.props.navigation;
    ...
    if (
      ...
      if (_userInfo.activeUserRoles.length <= 0) {
        ...
        if (userRole.role.name == 'user')
          ...
          navigate('TalentSeekerCategory', {
            sign_up_info: {fb_data: _userInfo},
          });
      }
    }
  };
          ...
          if (response) {
            ...
            if (response.code == 200) {
              ...
              if (!response.result.is_register_completed) {
                let _userDataSignUp = StorageData._saveUserData(
                  'SignUpProcess',
  ...
  forgetPassword = () => {
    GoogleAnalyticsHelper._trackEvent(
      'Log In',
    ...
    return (
      <View style={styles.container} onPress={() => dismissKeyboard()}>
        <TextInput
          ...
          style={[
            styles.input,
            this.state.email.isErrRequired && {color: 'red'},
          ]}
        ...
        <TouchableOpacity
          ...
          onPress={this._logInBtn}>
          {!this.state.emailLoginLoading ? (
            <Text style={styles.btn}>
              {this.props.loginFrm.state.btnLoginText}
            </Text>
          ) : (
