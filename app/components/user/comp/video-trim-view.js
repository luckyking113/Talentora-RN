...
class VideoTrimView extends Component {
  constructor(props) {
    super(props);
  ...
  trimVideo() {
    ...
    try {
      ...
      GoogleAnalyticsHelper._trackException(
        'Get Preview Image Video Trim (Trim Start Second) == ' +
          JSON.stringify(e),
      );
      try {
        ...
        GoogleAnalyticsHelper._trackException(
          'Get Preview Image Video Trim (0sec) == ' + JSON.stringify(_e),
        );
      }
    }

    ProcessingManager.trim(
      this._getVideoPathOrUrl(this.state.video.data),
      options,
    ) // like VideoPlayer trim options
      .then(videoTrimUri => {
  ...
  getVideoInfo(_videoUri = null) {
    ...
    try {
      ProcessingManager.getVideoInfo(
        _videoUri || this._getVideoPathOrUrl(this.state.video.data),
      )
        .then(({duration, size}) => {
          this.setState({
  ...
  _getTotalVideoDuration = (_start, _end) => {
    ...
    let _tolalDuration =
      ...
      (_start + (this.state.videoInfo.duration - _end));
  ...
  render() {
    ...
    try {
      return (
        <View
          style={[
            styles.justFlexContainer,
            {backgroundColor: 'black', paddingTop: 30},
          ]}>
          <View style={[{height: 60}]}>
            <Trimmer
              ...
              onChange={e => {
                console.log(
                  'Start :',
                  e.startTime * (Helper._isAndroid() ? 1000 : 1),
                  ' == ',
                  'End: ',
                  e.endTime * (Helper._isAndroid() ? 1000 : 1),
                  'Player: ',
                );
                this.player.seek(
                  e.startTime * (Helper._isAndroid() ? 1000 : 1),
          ...
          <View style={[styles.justFlexContainer, styles.middleSection]}>
            <TouchableOpacity
              ...
              style={[styles.justFlexContainer, styles.imgContainer]}>
              {/* { !Helper._isAndroid() &&  */}
              <Video
                source={{uri: this._getVideoPathOrUrl(this.state.video.data)}} // Can be a URL or a local file.                             // Store reference
                ref={ref => {
                  this.player = ref;
                }}
            ...
            {this.state.isPaused && !this.state.showLoading && (
              <TouchableOpacity
                onPress={() => this.toggleVideoPlay()}
                activeOpacity={0.8}
                style={[
            ...
            <View style={[styles.trimInfo]}>
              <View style={[styles.trimInfoContainer]}>
                ...
                <Text style={[styles.durationLabel]}>
                  Duration{' '}
                  {Helper.formatVideoDuration(this.state.totalVideoDuration)}{' '}
                  seconds
                </Text>
          ...
          <View style={[styles.trimAction]}>
            ...
            <TouchableOpacity
              ...
              style={[styles.btnTrim]}
              activeOpacity={0.8}>
              {!this.state.isTriming && (
                <Text style={[styles.btnText]}>Trim</Text>
              )}
              {this.state.isTriming && (
