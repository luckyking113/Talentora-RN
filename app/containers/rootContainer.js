...
class Root extends Component {
  ...
  onOpened(openResult) {
    ...
    DeviceEventEmitter.emit('OpenNotificationDetail', {
      notificationData: 'any-data',
    });
  }
    ...
    if (UserHelper._isLogIn()) {
      ...
      getApi('/api/devices/version').then(result => {
        ...
        if (result.code == 200 && result.result) {
          if (
            Helper._isIOS()
              ? result.result.ios_version > DeviceInfo.getVersion()
              : result.result.android_version > DeviceInfo.getBuildNumber()
          ) {
            Alert.alert(
              'Update Available',
              'A new update of Talentora is available. Please update to the lastest version.',
              [
                {
                  text: 'Next time',
                },
                {
                  ...
                  onPress: () => {
                    let url =
                      'https://play.google.com/store/apps/details?id=co.talentora.app';
                    if (Helper._isIOS()) {
                      url =
                        'https://itunes.apple.com/app/talentora/id1164383007';
                    }
                    Linking.openURL(url);
                  },
                },
    ...
    _userData.then(function(result) {
      ...
      if (!_.isEmpty(result)) {
        ...
        setTimeout(function() {
          ...
          if (UserHelper._checkUserExpiredToken()) {
  ...
  componentDidMount() {
    ...
    if (Helper._isAndroid()) OneSignal.clearOneSignalNotifications();
  }
